...
3. **Build Accessibility Server** (POC)
   ...
   - Test with existing `yarn access` script
...
2. **Performance Server**
   - Implement `analyze_bundle` tool (wraps `yarn analyze`)
...
yarn add --dev ts-morph ajv winston simple-git web-app-manifest-validator
```
