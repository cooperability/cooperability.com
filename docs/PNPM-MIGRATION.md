...
`next build --webpack` is a one-word revert. The App Router work in #267
pinned `--webpack` because Yarn PnP could not resolve `next`. This rebuild
drops that pin: pnpm's `node_modules` tree is what Turbopack can read.
...
`package.json` — and CI (`pnpm/action-setup` with no `version:` key) and every
developer machine read it from there.

Vercel is the exception. As of 2026-09-11, `build-utils` still does not list
pnpm 11 ([vercel/vercel#17434](https://github.com/vercel/vercel/issues/17434),
PR [#17450](https://github.com/vercel/vercel/pull/17450) unmerged). Lockfile
9.0 is then detected as pnpm 9, or the install falls through to npm. Setting
`ENABLE_EXPERIMENTAL_COREPACK=1` in `vercel.json` `env` is not enough: that
flag is applied after package-manager detection. The install command therefore
bootstraps pnpm 11 itself:

```
corepack enable && corepack prepare pnpm@11.24.0 --activate && pnpm install --frozen-lockfile
```

CI has a `Vercel-shaped install and build` job that runs that same command on
a stock Node image, without `pnpm/action-setup`, so a detector miss fails in
GitHub Actions instead of only on a preview URL.

Two other Vercel constraints:
...
- **`packages: ['.']` stays in `pnpm-workspace.yaml`.** pnpm 11 does not need
  it. pnpm 9 errors with `packages field missing or empty` if an old detector
  still launches.
...
7. **ESLint 10 is kept.** Rechecked on 2026-09-11 against this tree (Next 16.3.4,
   `eslint-config-next` 16.3.3, scoped APP files in `eslint.config.mjs` from
   #267). `pnpm lint` exits 0. The earlier 9.x downgrade was for the pre-#267
   config and is not replayed.
