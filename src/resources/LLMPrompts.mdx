...
<AccordionTrigger>Format output for Google Docs</AccordionTrigger>
<AccordionContent>
...
<AccordionItem value="cursorrules">
<AccordionTrigger>Exemplary .cursorrules Configuration</AccordionTrigger>
<AccordionContent>
...
- Enable pnpm with `corepack enable pnpm` before running package commands; the version is pinned by the `packageManager` field

## Dependencies & Setup

- Use pnpm (`pnpm-lock.yaml` exists) for package management; never npm or yarn
- Always check if node_modules exists before suggesting code fixes
- Common commands: `pnpm install`, `pnpm dev`, `pnpm build`, `pnpm typecheck`
...
3. Use `pnpm dev` to test changes locally
4. Lint with `pnpm lint` before suggesting code complete
...
1. Missing dependencies (suggest pnpm install)
...
<AccordionItem value="synthesize-transcript">
<AccordionTrigger>Synthesize Report from Long Transcript</AccordionTrigger>
<AccordionContent>
...
2. Bullet points, as many as you see fit, to enumerate the sub-points of each
   ...
   included verbatim among these bullet points, with links if applicable.
...
- ii. ANY external resource or tool that the speaker references or recommends,
  and rationale for why it was recommended.
...
<AccordionTrigger>Generate Product-Market Research</AccordionTrigger>
<AccordionContent>
...
<AccordionItem value="compose-prd">
<AccordionTrigger>Compose PRD from Source Codebase</AccordionTrigger>
<AccordionContent>
...
<AccordionItem value="summarize-chat">
<AccordionTrigger>Summarize this chat as a new prompt</AccordionTrigger>
<AccordionContent>
...
<AccordionItem value="summarize-book">
<AccordionTrigger>Summarize a Book</AccordionTrigger>
<AccordionContent>
...
required for a human to: 1. read the book, 2. take notes on it and, 3. if
applicable, complete any actionable projects within WITHOUT chatGPT and WITH
chatGPT. Your output should be in markdown, which should be readable by Anki,
GitHub, Next.js posts, etc. Format should be maximally universal where possible.
...
- **CS/IE** = Case Study or Illustrative Example: a story or narrative the text
  ...
  These should typically be paraphrased and marked with a page number for
  further investigation, but occasionally there are **CS/IE**s which can be
  entirely contained in a DQ of a few sentences. In these cases DQ is
  preferable.
...
- **Five-paragraph overall summary (PS)**
  ...
  - 3 paragraphs: three major movements from the work demonstrating its segues
    between major arguments, & important subpoints in each argument
...
1. CLEARLY mark what is (**DQ),** (**PS) and (CS/IE[either DQ or PS])** for all
   bullet points of the summary so as to make the summary properly attributable.
2. If uncertain about any part of the reduction/summarization process, ask me
   about specifics instead of assuming any particular behavior.
3. Points which are already too nuanced/concise to be effectively summarized
   should be marked with page numbers so that I can peruse the book.
