...
1. `.claude/cli/test/COMMANDS.md` — Jest
2. `.claude/cli/accessibility/COMMANDS.md` — when user wants axe / Lighthouse / ARIA
3. `.claude/cli/quality/COMMANDS.md` — lint/types/audit gates

(Use the `.cursor/cli/` twin if that tree is active.) If catalogs are missing, discover from `package.json` / CI. Prefer non-interactive runners.
